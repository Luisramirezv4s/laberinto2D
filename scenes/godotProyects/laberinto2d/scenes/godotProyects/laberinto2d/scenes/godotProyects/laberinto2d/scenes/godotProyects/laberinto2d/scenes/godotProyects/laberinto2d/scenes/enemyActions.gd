...
@onready var _dialog_scene := preload("res://UI/ProblemDialog.tscn")
var _dialog: WindowDialog      # instancia que crearemos
var _player_ref: Node2D        # referencia al jugador que choca
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	print("⚠️ Area2D detectó a: ", body.name)
	if not body.has_method("_on_HitEnemy"):
		return
		
		body._on_HitEnemy(position.x)
